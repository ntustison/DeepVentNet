...
hyperpolarized gas MR images [@Tustison:2011aa;@Tustison:2016aa].  We then describe the 
proposed deep learning methods (including preprocessing) extending earlier work.
Specifically, the proposed contributions include:
...
In [@Tustison:2016aa], we proposed a joint label fusion (JLF) approach for segmenting
...
results, one of the drawbacks is the time and computational resources required to 
perform the image registration for each member of the atlas set and the subsequent
voxelwise label estimation.   
...
via the expectation-maximization algorithm. Basically, the intensity histogram profile
...
the resulting estimated labels are then refined based an MRF spatial modeling.  The
parameters of the class-specific Gaussians are then re-estimated.  This iterative
process continues until convergence.  We also iterate this segmentation with application
of N4 bias correction [@Tustison:2010ac].

These approaches have been used in a number of clinical studies [@Altes:2016aa;@Altes:2017aa]
